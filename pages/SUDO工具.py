...
st.markdown("""
#### 使用说明：  
一种简化的up-down方法（SUDO），用于使用von-Frey丝测量啮齿动物的机械伤害刺激  
本工具是基于 von Frey 行为测试中常用的五根纤维丝（**0.16 g、0.4 g、0.6 g、1.0 g、2.0 g**）的反应组合规律所构建的阈值换算表。  
其结果基于对这些特定力度纤维丝的递进测试设计，**不适用于其他力度**（注意以上五根纤维丝中间跳过了 **1.4g** 的纤维丝）。  
...
共 **测试6次** ，若 **连续出现4次阳性（1111），或5次阴性（00000）** ，即停止测量
...
""")
st.markdown("由 0 表示阴性，1 表示阳性，例如输入序列 **000010**），可单个或多个查询。")
